...
import { seriesBookingSchema, type SeriesBookingInput } from "@/lib/validation/booking";
...
export function SeriesBookingForm({ rooms }: { rooms: Room[] }) {
  ...
  } = useForm<SeriesBookingInput>({
    resolver: zodResolver(seriesBookingSchema),
    defaultValues: {
      requesterName: "",
      requesterEmail: "",
      requesterPhone: "",
  ...
  async function onSubmit(values: SeriesBookingInput) {
    setServerError(null);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="requesterNameSeries">Name</Label>
          <Input id="requesterNameSeries" {...register("requesterName")} />
          {errors.requesterName ? (
            <p className="text-sm text-destructive">{errors.requesterName.message}</p>
          ) : null}
        </div>
        <div className="space-y-2">
          <Label htmlFor="requesterEmailSeries">E-Mail</Label>
          <Input id="requesterEmailSeries" type="email" {...register("requesterEmail")} />
          {errors.requesterEmail ? (
            <p className="text-sm text-destructive">{errors.requesterEmail.message}</p>
          ) : null}
        </div>
        <div className="space-y-2">
          <Label htmlFor="requesterPhoneSeries">Telefon (optional)</Label>
          <Input id="requesterPhoneSeries" {...register("requesterPhone")} />
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="space-y-2">
          <Label htmlFor="startTimeSeries">Startzeit</Label>
          <Input id="startTimeSeries" type="time" {...register("startTime")} />
          {errors.startTime ? (
            <p className="text-sm text-destructive">{errors.startTime.message}</p>
          ) : null}
        </div>
        <div className="space-y-2">
          <Label htmlFor="endTimeSeries">Endzeit</Label>
          <Input id="endTimeSeries" type="time" {...register("endTime")} />
          {errors.endTime ? (
            ...
            render={({ field }) => (
              <RadioGroup value={field.value} onValueChange={field.onChange} className="gap-3">
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="on_date" id="endType-date" />
                  <Label htmlFor="endType-date" className="font-normal">
                    an einem bestimmten Datum
                  </Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="after_count" id="endType-count" />
                  <Label htmlFor="endType-count" className="font-normal">
      ...
      <div className="space-y-2">
        <Label htmlFor="messageSeries">Nachricht (optional)</Label>
        <Textarea id="messageSeries" rows={4} {...register("message")} />
      </div>

      {serverError ? <p className="text-sm text-destructive">{serverError}</p> : null}

      <Button type="submit" disabled={isSubmitting}>
        {isSubmitting ? "Wird gesendet…" : "Serienanfrage senden"}
      </Button>
    </form>
  );
}
